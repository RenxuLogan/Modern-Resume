...
- **更改主题色**：定位到 `\definecolor{MyDarkBlue}{RGB}{0, 50, 100}`。
  - 想要**学术深红**？尝试：`{139, 0, 0}`
  - 想要**高级纯黑**？尝试：`{0, 0, 0}`
- **标题梯形与横线适配**：在 `\titleformat{\section}` 宏定义中，修改 `inner xsep=10pt` 即可调整标题文字左右的蓝色留白长度。
  - **⚠️ 重要提醒 (字数修改必看)**：横线起点的偏移量 `[xshift=-68.30pt]` 是针对**四个中文字符**（如“教育背景”）专门调校的。如果你修改了标题字数（例如改成两个中文字，或者英文单词），梯形的宽度会发生改变。此时，你需要同步调整 `\draw` 命令中的 `xshift` 数值（比如字数变少，数值绝对值变小），才能确保延伸横线与蓝色梯形完美衔接，避免断层或溢出。
